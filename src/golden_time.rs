...
pub fn first_day_year(year: i32) -> NaiveDate {
    NaiveDate::from_ymd_opt(year, 01, 01).unwrap()
}

pub fn last_day_year(year: i32) -> NaiveDate {
...
pub fn define_first_week(date: NaiveDate) -> Week {
...
pub fn define_last_week(date: NaiveDate) -> Week {
    let weekday: Weekday = date.weekday();
...
pub fn define_week(date: NaiveDate) -> Week {
...
// overlap
pub fn define_week_range(start_date: NaiveDate, end_date: NaiveDate) -> Vec<Week> {
...
pub fn define_weeks_in_year(start_date: NaiveDate, end_date: NaiveDate) -> Vec<Week> {
...
pub fn is_day_owned_by_month(day: NaiveDate, month: ChronoMonth) -> bool {
    day.month() == month.number_from_month()
}

pub fn how_many_days_in_week_owned_by_month(week: &Week, month: ChronoMonth) -> u8 {
...
pub fn iter_days(start_date: NaiveDate, end_date: NaiveDate) -> Vec<NaiveDate> {
...
pub fn split_in_month(weeks: Vec<Week>, month: ChronoMonth) -> Month {
    let weeks = weeks
        .into_iter()
...
pub fn split_in_months(weeks: Vec<Week>) -> Vec<Month> {
...
pub fn display_year(year: i32, months: Vec<Month>) -> () {
    println!("{}", year);
    for month in months {
